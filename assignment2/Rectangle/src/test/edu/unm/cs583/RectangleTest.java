...
public class RectangleTest {
	...
	public void testDiagonal() {
		...
		assertEquals(r.diagonal(), Math.sqrt(2), .01);
		try {
			r = new Rectangle(0, 0, -3, 4);
			r.diagonal();
			fail();
		} catch (IllegalArgumentException e) {
...
/* The following test methods were not written in class */	
	...
	@Test
    public void diagonalPrecisionTest(){
		Rectangle t = new Rectangle(0, 0, 8, 6);
		assertEquals(t.diagonal(), 10, 0);
		BigDecimal wolframSqrt = new BigDecimal("1.4142135623730950");
		t = new Rectangle(0,0,1,1);
		assertTrue(equalsWithBigDecimal(wolframSqrt, t.diagonal()));
		t = new Rectangle(0,0, 10000, 10001);
		wolframSqrt = new BigDecimal("14142.842748188922");
		assertTrue(equalsWithBigDecimal(wolframSqrt, t.diagonal()));
    }

    private boolean equalsWithBigDecimal(BigDecimal wolframSqrt, Double diagonal) {
    	return(wolframSqrt.compareTo(new BigDecimal(diagonal.toString())) == 0);   	 
    }
    ...
    public void intersectsTest() {
    	Rectangle t1 = new Rectangle(0, 0, 2, 2);
    	Rectangle t2 = new Rectangle(1, 1, 2, 2);
    	assertTrue(t1.intersects(t2));
    	assertTrue(t2.intersects(t1));
    	t1 = new Rectangle(0, 0, 5, 5);
    	t2 = new Rectangle(5, 5, 1, 1);
    	t2 = new Rectangle(6, 6, 1, 1);
    	assertFalse(t1.intersects(t2));
    	assertFalse(t2.intersects(t1));
    ...
    public void intersectionTest() {
    	Rectangle t1 = new Rectangle(0, 0, 2, 2);
    	Rectangle t2 = new Rectangle(1, 1, 2, 2);
    	Rectangle intersection = t1.intersection(t2);
    	assertTrue(intersection.equals(new Rectangle(1, 1, 1, 1)));
    }
